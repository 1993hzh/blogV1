...
public class IPSeeker {
	...
	public IPSeeker(String fileName, String dir) {
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(fileName, "r");
		} catch (FileNotFoundException e) {
			// 如果找不到这个文件，再尝试再当前目录下搜索，这次全部改用小写文件名  
			//     因为有些系统可能区分大小写导致找不到ip地址信息文件  
			String filename = new File(fileName).getName().toLowerCase();
				...
				if (files[i].isFile()) {
					if (files[i].getName().toLowerCase().equals(filename)) {
						try {
							...
							logger.debug("IP地址信息文件没有找到，IP显示功能将无法使用");
							ipFile = null;
						}
						break;
					}
		...
		if (ipFile != null) {
			try {
				...
				logger.debug("IP地址信息文件格式有错误，IP显示功能将无法使用");
				ipFile = null;
			}
		}
	}
